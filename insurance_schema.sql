...
CREATE TABLE CorporateClients (
    ...
    account_revenue_tier VARCHAR(20) NOT NULL 
        CHECK (account_revenue_tier IN ('Enterprise', 'Mid-Market', 'SMB')),
    risk_appetite_rating VARCHAR(20) NOT NULL 
        CHECK (risk_appetite_rating IN ('Risk-Averse', 'Balanced', 'Aggressive'))
);
...
CREATE TABLE InsurancePolicies (
    ...
    CONSTRAINT fk_policy_client 
        ...
        REFERENCES CorporateClients(client_id) 
        ON DELETE CASCADE
);
...
CREATE TABLE PolicyCoverages (
    ...
    contract_clause_reference VARCHAR(30) NOT NULL, -- Exact PDF index for audit validation
    coverage_term_raw VARCHAR(100) NOT NULL,        -- As written in the carrier contract
    normalized_taxonomy_group VARCHAR(50) NOT NULL,  -- Standardized mapping key for side-by-side grids
    aggregate_coverage_limit DECIMAL(15,2) NOT NULL,
    inner_sub_limit_cap DECIMAL(15,2) DEFAULT 0.00,
    
    CONSTRAINT fk_coverage_parent_policy 
...
CREATE TABLE PolicyExclusions (
    ...
    unmitigated_risk_severity VARCHAR(20) NOT NULL 
        CHECK (unmitigated_risk_severity IN ('Critical', 'Standard', 'Low')),
    
    CONSTRAINT fk_exclusion_parent_policy 
        FOREIGN KEY (policy_id) 
        REFERENCES InsurancePolicies(policy_id) 
        ON DELETE CASCADE
);
...
CREATE TABLE Recommendations (
    ...
    advisory_status VARCHAR(20) NOT NULL 
        CHECK (advisory_status IN ('Draft', 'Presented_To_Client', 'Bound_Confirmed')),
    final_broker_signoff_user VARCHAR(50) NOT NULL,
    
    CONSTRAINT fk_recommendation_client 
        FOREIGN KEY (client_id) 
        REFERENCES CorporateClients(client_id) 
        ON DELETE CASCADE,
    CONSTRAINT fk_recommendation_policy 
        FOREIGN KEY (selected_policy_id) 
        REFERENCES InsurancePolicies(policy_id)
);
...
CREATE TABLE AuditLogs (
    ...
    CONSTRAINT fk_audit_parent_policy 
        FOREIGN KEY (policy_id) 
        REFERENCES InsurancePolicies(policy_id) 
        ON DELETE CASCADE
);
